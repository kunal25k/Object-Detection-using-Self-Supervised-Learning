...
class UnlabeledDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        idx2 = idx+512001
        with open(os.path.join(self.image_dir, f"{idx2}.PNG"), 'rb') as f:
            try:	
            	img = Image.open(f).convert('RGB')
            except:
                #self.num_images = self.num_images - 1
                print("exception found, loading previous image" )
                idx3 = idx2-1
                with open(os.path.join(self.image_dir, f"{idx3}.PNG"), 'rb' ) as f1:
