...
class Model():
    ...
    def train(self, num_epochs: int, gpu=None, sampler=None) -> None:
        """
        ...
        for epoch in tqdm(range(num_epochs)):
            for phase in ['train']:
                ...
                if epoch_stats[0] > self.best_stat:
    ...
    def log_stats(self, epoch: int, losses: List[Dict], stats: List[Dict] = [], data: List = [], lr_rates: List = []):
        ...
        if epoch == 0:
            metrices_title = '\t'.join(
                [(phase + '_' + mat)[:12] for mat in self.metrices for phase in ['train']])
            tqdm.write('Epoch\tTrain_Loss\t' + metrices_title)
            tqdm.write(
                '-----------------------------------------------------------------------------------------------------')
        metrices_stats = ' \t '.join([format(stats[phase][epoch][i], '.6f') for i in range(len(self.metrices))
                                      for phase in ['train']])
        log_stats = ' \t '.join([format(losses[phase][epoch], '.6f') for phase in ['train']])
        tqdm.write('{:4d} \t'.format(epoch + 1) + log_stats)
